...
public class RedisConfiguration {
	@Value("${redis.linux.hostname}")
	private String linuxHostname;
	@Value("${redis.win.hostname}")
	private String winHostName;
	@Value("${redis.port}")
	private int port;
	...
	public String gethost() {
		...
		if (os.toLowerCase().startsWith("win")) {
			basePath = winHostName;
		} else {
			basePath = linuxHostname;
		}
		return basePath;
	}
	/**
	...
	public JedisPoolWriper createJedisPoolWriper() {
		return new JedisPoolWriper(jedisPoolConfig, gethost(), port);
	}
