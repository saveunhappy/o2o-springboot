...
public class WechatLoginController {
	//前端展示系统还是后台系统的flag，店铺就返回2，用户就返回1
	...
	public String doGet(HttpServletRequest request, HttpServletResponse response) {
		...
		if (null != code) {
			...
			try {
				...
				// 通过access_token和openId获取用户昵称等信息，然后通过jackson解析，就返回回来user的信息了
		...
		if (auth == null) {
			PersonInfo personInfo = WechatUtil.getPersonInfoFromRequest(Objects.requireNonNull(user));
			auth = new WechatAuth();
